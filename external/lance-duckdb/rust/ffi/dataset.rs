...
fn dataset_generation_identity(dataset: &lance::Dataset, stable_root: bool) -> String {
    let manifest = dataset.manifest();
    let location = dataset.manifest_location();
    format!(
        "{}|{}|{}|{}|{}|{}",
        if stable_root { "root" } else { "current" },
        manifest.version,
...
pub unsafe extern "C" fn lance_dataset_generation_id(dataset: *mut c_void) -> *const c_char {
    ...
    // Version 1 is invariant for the lifetime of a dataset generation. Vacuum
    // may remove that manifest, in which case a version-specific identity is a
    // conservative fallback: it can reduce cache reuse but cannot join two
    // different generations or return a stale handle.
    let generation_id = match runtime::block_on(handle.dataset.checkout_version(1)) {
        Ok(Ok(root_dataset)) => dataset_generation_identity(&root_dataset, true),
        Ok(Err(_)) | Err(_) => dataset_generation_identity(&handle.dataset, false),
    };
    clear_last_error();
