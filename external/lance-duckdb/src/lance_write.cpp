...
struct LanceWriteBindData : public FunctionData {
	...
	bool Equals(const FunctionData &other_p) const override {
		...
		return mode == other.mode && data_storage_version == other.data_storage_version &&
		       max_rows_per_file == other.max_rows_per_file && max_rows_per_group == other.max_rows_per_group &&
		       max_bytes_per_file == other.max_bytes_per_file && names == other.names && types == other.types;
	}
};
...
static void LanceWriteCopyOptions(ClientContext &, CopyOptionsInput &input) {
	...
	options["mode"] = CopyOption(LogicalType::VARCHAR, CopyOptionMode::WRITE_ONLY);
	options["data_storage_version"] = CopyOption(LogicalType::VARCHAR, CopyOptionMode::WRITE_ONLY);
	options["max_rows_per_file"] = CopyOption(LogicalType::UBIGINT, CopyOptionMode::WRITE_ONLY);
	options["max_rows_per_group"] = CopyOption(LogicalType::UBIGINT, CopyOptionMode::WRITE_ONLY);
	options["max_bytes_per_file"] = CopyOption(LogicalType::UBIGINT, CopyOptionMode::WRITE_ONLY);
}

static unique_ptr<FunctionData> LanceWriteBind(ClientContext &, CopyFunctionBindInput &input,
	...
	for (auto &option : input.info.options) {
		...
		if (option.second.size() != 1) {
			throw BinderException("%s requires exactly one argument", StringUtil::Upper(key));
		}
		...
		if (key == "mode") {
			...
			if (mode != "create" && mode != "append" && mode != "overwrite") {
				throw BinderException("mode must be one of [create, append, overwrite]");
			}
...
static void LanceWriteSerialize(Serializer &serializer, const FunctionData &bind_data_p, const CopyFunction &) {
	auto &bind_data = bind_data_p.Cast<LanceWriteBindData>();
	serializer.WriteProperty(100, "mode", bind_data.mode);
	serializer.WriteProperty(101, "data_storage_version", bind_data.data_storage_version);
	serializer.WriteProperty(102, "max_rows_per_file", bind_data.max_rows_per_file);
	serializer.WriteProperty(103, "max_rows_per_group", bind_data.max_rows_per_group);
	serializer.WriteProperty(104, "max_bytes_per_file", bind_data.max_bytes_per_file);
	serializer.WriteProperty(105, "names", bind_data.names);
	serializer.WriteProperty(106, "types", bind_data.types);
}

static unique_ptr<FunctionData> LanceWriteDeserialize(Deserializer &deserializer, CopyFunction &) {
	auto result = make_uniq<LanceWriteBindData>();
	result->mode = deserializer.ReadProperty<string>(100, "mode");
	result->data_storage_version = deserializer.ReadProperty<string>(101, "data_storage_version");
	result->max_rows_per_file = deserializer.ReadProperty<uint64_t>(102, "max_rows_per_file");
	result->max_rows_per_group = deserializer.ReadProperty<uint64_t>(103, "max_rows_per_group");
	result->max_bytes_per_file = deserializer.ReadProperty<uint64_t>(104, "max_bytes_per_file");
...
static unique_ptr<GlobalFunctionData> LanceWriteInitGlobal(ClientContext &context, FunctionData &bind_data_p,
                                                           const string &file_path) {
	auto &bind_data = bind_data_p.Cast<LanceWriteBindData>();
	auto state = make_uniq<LanceWriteGlobalState>();

	auto props = context.GetClientProperties();
	memset(&state->schema_root.arrow_schema, 0, sizeof(state->schema_root.arrow_schema));
	ArrowConverter::ToArrowSchema(&state->schema_root.arrow_schema, bind_data.types, bind_data.names, props);

	vector<string> option_keys;
	vector<string> option_values;
	string open_path;
	ResolveLanceStorageOptions(context, file_path, open_path, option_keys, option_values);
	state->cache_key = LanceBuildResolvedPathDatasetCacheKey(open_path, option_keys, option_values);
	...
	const char *data_storage_version_ptr =
	    bind_data.data_storage_version.empty() ? nullptr : bind_data.data_storage_version.c_str();
	auto *session = LanceGetSessionHandle(context);
	state->writer = lance_open_writer_with_storage_options(
	    open_path.c_str(), bind_data.mode.c_str(), key_ptrs.empty() ? nullptr : key_ptrs.data(),
	    value_ptrs.empty() ? nullptr : value_ptrs.data(), option_keys.size(), bind_data.max_rows_per_file,
	    bind_data.max_rows_per_group, bind_data.max_bytes_per_file, data_storage_version_ptr, session,
...
static unique_ptr<LocalFunctionData> LanceWriteInitLocal(ExecutionContext &, FunctionData &) {
	return make_uniq<LanceWriteLocalState>();
}

static void LanceWriteSink(ExecutionContext &context, FunctionData &, GlobalFunctionData &gstate_p, LocalFunctionData &,
                           DataChunk &input) {
	if (input.size() == 0) {
		return;
	}
	...
	if (rc != 0) {
		throw IOException("Failed to write to Lance dataset" + LanceFormatErrorSuffix());
	}
}

static void LanceWriteFinalize(ClientContext &context, FunctionData &, GlobalFunctionData &gstate_p) {
	auto &gstate = gstate_p.Cast<LanceWriteGlobalState>();
	if (!gstate.writer) {
		return;
	}
...
static CopyFunctionExecutionMode LanceWriteExecutionMode(bool, bool) {
	return CopyFunctionExecutionMode::SINGLE_COMMIT_WRITER;
}
