...
static constexpr const char *LANCE_SEARCH_GLOBAL_SPLIT = "lance-search-global-v1";

static vector<ExtensionScanSplit> LanceGlobalSearchSplit(uint64_t k) {
	ExtensionScanSplit split;
	split.payload = LANCE_SEARCH_GLOBAL_SPLIT;
	split.estimated_cardinality = NumericCast<idx_t>(k);
	return {std::move(split)};
}

static void ValidateLanceGlobalSearchSplits(const vector<string> &splits) {
	if (splits.size() != 1 || splits[0] != LANCE_SEARCH_GLOBAL_SPLIT) {
		throw SerializationException("Lance search requires exactly one global search split");
	}
}

static void PopulateSearchSchemaFromTypes(ClientContext &context, ArrowSchemaWrapper &schema_root,
                                          ArrowTableSchema &arrow_table, const vector<string> &names,
                                          const vector<LogicalType> &types) {
	memset(&schema_root.arrow_schema, 0, sizeof(schema_root.arrow_schema));
	auto properties = context.GetClientProperties();
	ArrowConverter::ToArrowSchema(&schema_root.arrow_schema, types, names, properties);
...
static bool TryLanceExplainKnn(void *dataset, const string &vector_column, const vector<float> &query, uint64_t k,
                               uint64_t nprobes, uint64_t refine_factor, const string *filter_ir, bool prefilter,
                               bool use_index, bool verbose, string &out_plan, string &out_error) {
	out_plan.clear();
	out_error.clear();
	...
	auto *plan_ptr = lance_explain_knn_scan_ir(dataset, vector_column.c_str(), query.data(), query.size(), k, nprobes,
...
static shared_ptr<LanceDatasetCacheEntry> OpenSearchDatasetEntry(ClientContext &context, const Value &input,
                                                                 const string &function_name, string &out_display_uri,
                                                                 bool *out_cache_hit) {
	out_display_uri = ResolveLanceDatasetUri(context, input, LanceResolvePolicy::FALLBACK_TO_PATH, function_name);
	...
	if (auto *table = TryResolveLanceTableEntry(context, input_str)) {
		if (StringUtil::CIEquals(out_display_uri, table->DatasetUri())) {
			return LanceGetOrOpenDatasetEntryForTable(context, *table, out_display_uri, out_cache_hit);
		}
	}

	auto entry = LanceGetOrOpenDatasetEntry(context, out_display_uri, out_cache_hit);
	if (entry) {
...
static vector<float> ParseQueryVector(const Value &value, const string &function_name) {
	if (value.IsNull()) {
	...
	if (value.type().id() != LogicalTypeId::LIST && value.type().id() != LogicalTypeId::ARRAY) {
		throw InvalidInputException(function_name + " requires query vector to be a LIST or ARRAY");
	}
	...
	if (children.empty()) {
		throw InvalidInputException(function_name + " requires a non-empty query vector");
	}

	auto cast_f32 = [&function_name](double v) {
		if (!std::isfinite(v)) {
			throw InvalidInputException(function_name + " query vector contains non-finite value");
		}
		auto max_v = static_cast<double>(std::numeric_limits<float>::max());
		if (v > max_v || v < -max_v) {
			throw InvalidInputException(function_name + " query vector value is out of float32 range");
		}
	...
	for (auto &child : children) {
		...
		default:
			try {
				...
				throw InvalidInputException(function_name + " query vector elements must be numeric");
			}
		}
	}
	return out;
...
static string ParseOptionalNamedString(const TableFunctionBindInput &input, const string &name) {
...
static LanceTableEntry *TryResolveNamespaceBackedSearchTable(ClientContext &context, const Value &input) {
	auto input_str = input.GetValue<string>();
...
static string RequireNamespaceSearchColumn(const LanceTableEntry &table, const string &column,
	...
	throw InvalidInputException(function_name + " requires " + argument_name + " to name an existing column on " +
	                            "namespace-backed table: " + column);
}

static void PopulateNamespaceSearchSchema(ClientContext &context, const LanceTableEntry &table,
	...
	ArrowConverter::ToArrowSchema(&schema_root.arrow_schema, field_types, field_names, props);
	LanceCoerceArrowSchemaForDuckDB(&schema_root.arrow_schema);
	ArrowTableFunction::PopulateArrowTableSchema(context, arrow_table, schema_root.arrow_schema);
...
struct LanceKnnBindData : public TableFunctionData, public ExtensionScanSplitProvider {
	string file_path;
	...
	vector<ExtensionScanSplit> GetScanSplits() const override {
		return LanceGlobalSearchSplit(k);
	}

	void SetScanSplits(const vector<string> &splits) override {
		ValidateLanceGlobalSearchSplits(splits);
	}
...
struct LanceKnnLocalState : public ArrowScanLocalState {
	explicit LanceKnnLocalState(unique_ptr<ArrowArrayWrapper> current_chunk, ClientContext &context)
...
static void LancePushdownComplexFilter(ClientContext &, LogicalGet &get, FunctionData *bind_data,
	...
	for (auto &expr : filters) {
		...
		string filter_ir;
		if (!TryBuildLanceExprFilterIR(get, scan_bind.names, scan_bind.types, true, *expr, filter_ir)) {
			continue;
		}
...
static bool LancePushdownExpression(ClientContext &, const LogicalGet &, Expression &expr) {
	...
	auto &cmp = expr.Cast<BoundComparisonExpression>();
	return cmp.type == ExpressionType::COMPARE_DISTINCT_FROM || cmp.type == ExpressionType::COMPARE_NOT_DISTINCT_FROM;
}

static unique_ptr<FunctionData> LanceSearchVectorBind(ClientContext &context, TableFunctionBindInput &input,
                                                      vector<LogicalType> &return_types, vector<string> &names) {
	if (input.inputs.size() < 3) {
		throw InvalidInputException("lance_vector_search requires (path, vector_column, vector)");
	}
	if (input.inputs[0].IsNull()) {
		throw InvalidInputException("lance_vector_search requires a dataset root path");
	}
	if (input.inputs[1].IsNull()) {
		throw InvalidInputException("lance_vector_search requires a non-null vector_column");
	}
	if (input.inputs[2].IsNull()) {
		throw InvalidInputException("lance_vector_search requires a non-null query vector");
	}
	...
	if (k_named != input.named_parameters.end() && !k_named->second.IsNull()) {
		k_val = k_named->second.DefaultCastAs(LogicalType::BIGINT).GetValue<int64_t>();
	}
	if (k_val <= 0) {
	...
	if (nprobes_named != input.named_parameters.end() && !nprobes_named->second.IsNull()) {
		has_nprobes = true;
		nprobes_val = nprobes_named->second.DefaultCastAs(LogicalType::BIGINT).GetValue<int64_t>();
	}
	...
	if (refine_factor_named != input.named_parameters.end() && !refine_factor_named->second.IsNull()) {
		has_refine_factor = true;
		refine_factor_val = refine_factor_named->second.DefaultCastAs(LogicalType::BIGINT).GetValue<int64_t>();
	}
	if (has_refine_factor && refine_factor_val <= 0) {
		throw InvalidInputException("lance_vector_search requires refine_factor > 0");
	}
	result->refine_factor = has_refine_factor ? NumericCast<uint64_t>(refine_factor_val) : 0;

	auto prefilter_named = input.named_parameters.find("prefilter");
	if (prefilter_named != input.named_parameters.end() && !prefilter_named->second.IsNull()) {
		result->prefilter = prefilter_named->second.DefaultCastAs(LogicalType::BOOLEAN).GetValue<bool>();
	}
	auto use_index_named = input.named_parameters.find("use_index");
	if (use_index_named != input.named_parameters.end() && !use_index_named->second.IsNull()) {
		result->use_index = use_index_named->second.DefaultCastAs(LogicalType::BOOLEAN).GetValue<bool>();
	}

	if (auto *table = TryResolveNamespaceBackedSearchTable(context, input.inputs[0])) {
		...
		result->vector_column =
		    RequireNamespaceSearchColumn(*table, result->vector_column, "lance_vector_search", "vector_column");
		if (result->prefilter && result->namespace_filter.empty()) {
			throw InvalidInputException("lance_vector_search requires explicit filter when prefilter=true "
			                            "on namespace-backed tables");
		}
		PopulateNamespaceSearchSchema(context, *table, "_distance", result->schema_root, result->arrow_table,
		                              result->names, result->types, names, return_types);
		return std::move(result);
	}

	if (!result->namespace_filter.empty()) {
		throw InvalidInputException("lance_vector_search filter parameter is only supported for "
		                            "namespace-backed tables");
	}

	result->file_path.clear();
	result->dataset_entry = OpenSearchDatasetEntry(context, input.inputs[0], "lance_vector_search", result->file_path,
	...
	if (result->dataset_version == 0) {
		throw IOException("Failed to capture Lance dataset version: " + result->file_path + LanceFormatErrorSuffix());
	}

	auto *schema_handle = lance_get_knn_schema(result->dataset, result->vector_column.c_str(), result->query.data(),
	                                           result->query.size(), result->k, result->nprobes, result->refine_factor,
	                                           result->prefilter ? 1 : 0, result->use_index ? 1 : 0);
	if (!schema_handle) {
		throw IOException("Failed to get Lance KNN schema: " + result->file_path + LanceFormatErrorSuffix());
	}
	...
	if (lance_schema_to_arrow(schema_handle, &result->schema_root.arrow_schema) != 0) {
		...
		throw IOException("Failed to export Lance KNN schema to Arrow C Data Interface" + LanceFormatErrorSuffix());
	}
	lance_free_schema(schema_handle);
...
static unique_ptr<GlobalTableFunctionState> LanceKnnInitGlobal(ClientContext &, TableFunctionInitInput &input) {
	...
	auto table_filters = BuildLanceTableFilterIRParts(bind_data.names, bind_data.types, input, true);
	...
	if (bind_data.prefilter && has_table_filter_parts && global.lance_filter_ir.empty()) {
		throw IOException("Failed to encode Lance filter IR");
	}
	global.filter_pushed_down = table_filters.all_filters_pushed && !global.lance_filter_ir.empty();
	return state;
}

static unique_ptr<LocalTableFunctionState> LanceKnnLocalInit(ExecutionContext &context, TableFunctionInitInput &input,
	...
	if (bind_data.namespace_backed) {
		...
		FillLanceNamespaceQueryConfig(context.client, bind_data.namespace_config, bind_data.k, bind_data.prefilter,
		                              bind_data.namespace_filter, global.namespace_columns, resolved_option_keys,
		                              resolved_option_values, option_key_ptrs, option_value_ptrs, column_ptrs,
		                              bearer_token, api_key, headers_tsv, config);
		...
		result->stream = lance_create_namespace_vector_search_stream(&config, &options);
		if (!result->stream) {
	...
	const uint8_t *filter_ir =
	    global.lance_filter_ir.empty() ? nullptr : reinterpret_cast<const uint8_t *>(global.lance_filter_ir.data());
	auto filter_ir_len = global.lance_filter_ir.size();
	result->stream =
	    lance_create_knn_stream_ir(bind_data.dataset, bind_data.vector_column.c_str(), bind_data.query.data(),
	                               bind_data.query.size(), bind_data.k, bind_data.nprobes, bind_data.refine_factor,
	                               filter_ir, filter_ir_len, bind_data.prefilter ? 1 : 0, bind_data.use_index ? 1 : 0);
	...
	if (!result->stream) {
		throw IOException("Failed to create Lance KNN stream" + LanceFormatErrorSuffix());
	}
...
static void LanceKnnFunc(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	if (!data.local_state) {
		return;
	}
	...
	while (true) {
		...
		if (global_state.CanRemoveFilterColumns()) {
			...
			ArrowTableFunction::ArrowToDuckDB(local_state, bind_data.arrow_table.GetColumns(), local_state.all_columns,
			                                  false);
...
static InsertionOrderPreservingMap<string> LanceKnnToString(TableFunctionToStringInput &input) {
	InsertionOrderPreservingMap<string> result;
	auto &bind_data = input.bind_data->Cast<LanceKnnBindData>();

	result["Lance Path"] = bind_data.file_path;
	result["Lance Search Backend"] = bind_data.namespace_backed ? "namespace_query_table" : "dataset_scan";
	...
	string plan;
	string error;
	if (TryLanceExplainKnn(bind_data.dataset, bind_data.vector_column, bind_data.query, bind_data.k, bind_data.nprobes,
	                       bind_data.refine_factor, filter_ir_msg.empty() ? nullptr : &filter_ir_msg,
	                       bind_data.prefilter, bind_data.use_index, bind_data.explain_verbose, plan, error)) {
...
static InsertionOrderPreservingMap<string> LanceKnnDynamicToString(TableFunctionDynamicToStringInput &input) {
	InsertionOrderPreservingMap<string> result;
	auto &bind_data = input.bind_data->Cast<LanceKnnBindData>();
	...
	result["Lance Filter Pushed Down"] = global_state.filter_pushed_down ? "true" : "false";
	...
	if (!global_state.explain_computed.load()) {
		...
		if (!global_state.explain_computed.load()) {
			...
			auto ok = TryLanceExplainKnn(
			    bind_data.dataset, bind_data.vector_column, bind_data.query, bind_data.k, bind_data.nprobes,
			    bind_data.refine_factor, global_state.lance_filter_ir.empty() ? nullptr : &global_state.lance_filter_ir,
			    bind_data.prefilter, bind_data.use_index, bind_data.explain_verbose, plan, error);
			if (ok) {
...
static void LanceKnnSerialize(Serializer &serializer, const optional_ptr<FunctionData> bind_data_p,
                              const TableFunction &) {
	...
	serializer.WriteProperty(114, "lance_filter_ir_parts", bind_data.lance_pushed_filter_ir_parts);
	if (bind_data.namespace_backed) {
		auto &cfg = bind_data.namespace_config;
		serializer.WriteProperty(115, "namespace_kind", static_cast<uint8_t>(cfg.kind));
		...
		serializer.WriteProperty(121, "namespace_replay_secret_name", cfg.replay_secret_name);
	}
}

static unique_ptr<FunctionData> LanceKnnDeserialize(Deserializer &deserializer, TableFunction &) {
	auto result = make_uniq<LanceKnnBindData>();
	auto &context = deserializer.Get<ClientContext &>();
	result->task_cpu_slots = MaxValue<idx_t>(1, context.db->NumberOfThreads());
	result->file_path = deserializer.ReadProperty<string>(100, "file_path");
	result->vector_column = deserializer.ReadProperty<string>(101, "vector_column");
	...
	result->refine_factor = deserializer.ReadProperty<uint64_t>(105, "refine_factor");
	...
	result->explain_verbose = deserializer.ReadProperty<bool>(108, "explain_verbose");
	result->namespace_backed = deserializer.ReadProperty<bool>(109, "namespace_backed");
	result->namespace_filter = deserializer.ReadProperty<string>(110, "namespace_filter");
	result->dataset_version = deserializer.ReadProperty<uint64_t>(111, "dataset_version");
	...
	result->lance_pushed_filter_ir_parts = deserializer.ReadProperty<vector<string>>(114, "lance_filter_ir_parts");
	if (result->namespace_backed) {
		auto &cfg = result->namespace_config;
		cfg.kind = static_cast<LanceNamespaceKind>(deserializer.ReadProperty<uint8_t>(115, "namespace_kind"));
		...
		cfg.delimiter = deserializer.ReadProperty<string>(119, "namespace_delimiter");
		cfg.display_uri = deserializer.ReadProperty<string>(120, "namespace_display_uri");
		cfg.replay_secret_name = deserializer.ReadProperty<string>(121, "namespace_replay_secret_name");
		// Storage options, auth values and headers remain outside the plan.
	} else {
		if (result->dataset_version == 0) {
			throw SerializationException("Serialized Lance vector search is missing its fixed dataset version");
		}
		...
		if (!result->dataset || lance_dataset_version(result->dataset) != result->dataset_version) {
			throw IOException("Failed to reopen fixed Lance vector-search snapshot: " + result->file_path +
			                  LanceFormatErrorSuffix());
		}
	}
	PopulateSearchSchemaFromTypes(context, result->schema_root, result->arrow_table, result->names, result->types);
	return std::move(result);
}
...
static void RegisterLanceVectorSearch(ExtensionLoader &loader) {
	...
	TableFunction search_f32("lance_vector_search",
	                         ...
	                         LanceKnnFunc, LanceSearchVectorBind, LanceKnnInitGlobal, LanceKnnLocalInit);
	configure(search_f32);
	loader.RegisterFunction(search_f32);

	TableFunction search_f64("lance_vector_search",
	                         {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::LIST(LogicalType::DOUBLE)},
	                         LanceKnnFunc, LanceSearchVectorBind, LanceKnnInitGlobal, LanceKnnLocalInit);
...
struct LanceSearchBindData : public TableFunctionData, public ExtensionScanSplitProvider {
...
struct LanceSearchLocalState : public ArrowScanLocalState {
	explicit LanceSearchLocalState(unique_ptr<ArrowArrayWrapper> current_chunk, ClientContext &context)
	    : ArrowScanLocalState(std::move(current_chunk), context), filter_sel(STANDARD_VECTOR_SIZE) {
	}
...
static bool LanceSearchLoadNextBatch(ClientContext &context, LanceSearchLocalState &local_state,
                                     const LanceSearchBindData &bind_data, LanceSearchGlobalState &global) {
	if (!local_state.stream) {
		if (bind_data.namespace_backed) {
			vector<string> resolved_option_keys;
			vector<string> resolved_option_values;
			...
			string headers_tsv;
			LanceNamespaceQueryConfig config;
			FillLanceNamespaceQueryConfig(context, bind_data.namespace_config, bind_data.k, bind_data.prefilter,
			                              bind_data.namespace_filter, global.namespace_columns, resolved_option_keys,
			                              resolved_option_values, option_key_ptrs, option_value_ptrs, column_ptrs,
			                              bearer_token, api_key, headers_tsv, config);
			...
			local_state.stream = lance_create_namespace_fts_search_stream(&config, &options);
			if (!local_state.stream) {
				throw IOException("Failed to create Lance namespace FTS stream" + LanceFormatErrorSuffix());
			}
		} else {
			...
			auto create_stream = [&](const uint8_t *ir, idx_t ir_len) -> void * {
				if (bind_data.mode == LanceSearchMode::Fts) {
					return lance_create_fts_stream_ir(bind_data.dataset, bind_data.text_column.c_str(),
				...
				return lance_create_hybrid_stream_ir(
				    bind_data.dataset, bind_data.vector_column.c_str(), bind_data.vector_query.data(),
				    bind_data.vector_query.size(), bind_data.text_column.c_str(), bind_data.text_query.c_str(),
				    bind_data.k, bind_data.nprobes, bind_data.refine_factor, ir, NumericCast<size_t>(ir_len),
				    bind_data.prefilter ? 1 : 0, bind_data.use_index ? 1 : 0, bind_data.alpha,
			...
			if (!local_state.stream) {
				throw IOException("Failed to create Lance search stream" + LanceFormatErrorSuffix());
			}
		}
	}
...
static unique_ptr<FunctionData> LanceFtsBind(ClientContext &context, TableFunctionBindInput &input,
                                             vector<LogicalType> &return_types, vector<string> &names) {
	if (input.inputs.size() < 3) {
	...
	if (auto *table = TryResolveNamespaceBackedSearchTable(context, input.inputs[0])) {
		...
		result->namespace_config = table->NamespaceConfig();
		result->file_path = table->DatasetUri();
		result->text_column = RequireNamespaceSearchColumn(*table, result->text_column, "lance_fts", "text_column");
		if (result->prefilter && result->namespace_filter.empty()) {
			throw InvalidInputException("lance_fts requires explicit filter when prefilter=true on "
			                            "namespace-backed tables");
		}
		PopulateNamespaceSearchSchema(context, *table, "_score", result->schema_root, result->arrow_table,
		                              result->names, result->types, names, return_types);
		return std::move(result);
	}

	if (!result->namespace_filter.empty()) {
		throw InvalidInputException("lance_fts filter parameter is only supported for namespace-backed "
		                            "tables");
	}

	result->file_path.clear();
	result->dataset_entry =
	    OpenSearchDatasetEntry(context, input.inputs[0], "lance_fts", result->file_path, &result->dataset_cache_hit);
	result->dataset = result->dataset_entry ? result->dataset_entry->Handle() : nullptr;
	...
	auto *schema_handle = lance_get_fts_schema(result->dataset, result->text_column.c_str(), result->query.c_str(),
	                                           result->k, result->prefilter ? 1 : 0);
	if (!schema_handle) {
		throw IOException("Failed to get Lance FTS schema: " + result->file_path + LanceFormatErrorSuffix());
	}

	memset(&result->schema_root.arrow_schema, 0, sizeof(result->schema_root.arrow_schema));
	if (lance_schema_to_arrow(schema_handle, &result->schema_root.arrow_schema) != 0) {
		lance_free_schema(schema_handle);
		throw IOException("Failed to export Lance FTS schema to Arrow C Data Interface" + LanceFormatErrorSuffix());
	}
	lance_free_schema(schema_handle);
	LanceCoerceArrowSchemaForDuckDB(&result->schema_root.arrow_schema);
	ArrowTableFunction::PopulateArrowTableSchema(context, result->arrow_table, result->schema_root.arrow_schema);
...
static unique_ptr<FunctionData> LanceHybridBind(ClientContext &context, TableFunctionBindInput &input,
                                                vector<LogicalType> &return_types, vector<string> &names) {
	...
	if (input.inputs[0].IsNull()) {
		throw InvalidInputException("lance_hybrid_search requires a dataset root path");
	}
	if (input.inputs[1].IsNull()) {
		throw InvalidInputException("lance_hybrid_search requires a non-null vector_column");
	}
	if (input.inputs[2].IsNull()) {
		throw InvalidInputException("lance_hybrid_search requires a non-null query vector");
	}
	if (input.inputs[3].IsNull()) {
		throw InvalidInputException("lance_hybrid_search requires a non-null text_column");
	}
	if (input.inputs[4].IsNull()) {
		throw InvalidInputException("lance_hybrid_search requires a non-null query");
	}
	if (auto *table = TryResolveNamespaceBackedSearchTable(context, input.inputs[0]);
	    table && table->NamespaceConfig().IsRest()) {
		throw NotImplementedException(
		    "Lance hybrid search is not supported for REST namespace-backed tables; use vector or FTS search until "
	...
	result->file_path.clear();
	result->dataset_entry = OpenSearchDatasetEntry(context, input.inputs[0], "lance_hybrid_search", result->file_path,
	                                               &result->dataset_cache_hit);
	result->dataset = result->dataset_entry ? result->dataset_entry->Handle() : nullptr;
	result->vector_column = input.inputs[1].GetValue<string>();
	...
	if (has_refine_factor && refine_factor_val <= 0) {
		throw InvalidInputException("lance_hybrid_search requires refine_factor > 0");
	}
	result->refine_factor = has_refine_factor ? NumericCast<uint64_t>(refine_factor_val) : 0;
	...
	if (alpha_named != input.named_parameters.end() && !alpha_named->second.IsNull()) {
		result->alpha = alpha_named->second.DefaultCastAs(LogicalType::FLOAT).GetValue<float>();
	}
	auto oversample_named = input.named_parameters.find("oversample_factor");
	if (oversample_named != input.named_parameters.end() && !oversample_named->second.IsNull()) {
		auto v = oversample_named->second.DefaultCastAs(LogicalType::INTEGER).GetValue<int32_t>();
		if (v > 0) {
	...
	if (!schema_handle) {
		throw IOException("Failed to get Lance hybrid schema: " + result->file_path + LanceFormatErrorSuffix());
	}

	memset(&result->schema_root.arrow_schema, 0, sizeof(result->schema_root.arrow_schema));
	if (lance_schema_to_arrow(schema_handle, &result->schema_root.arrow_schema) != 0) {
		lance_free_schema(schema_handle);
		throw IOException("Failed to export Lance hybrid schema to Arrow C Data Interface" + LanceFormatErrorSuffix());
	}
	lance_free_schema(schema_handle);
	LanceCoerceArrowSchemaForDuckDB(&result->schema_root.arrow_schema);
	ArrowTableFunction::PopulateArrowTableSchema(context, result->arrow_table, result->schema_root.arrow_schema);
...
static unique_ptr<GlobalTableFunctionState> LanceSearchInitGlobal(ClientContext &, TableFunctionInitInput &input) {
	auto &bind_data = input.bind_data->Cast<LanceSearchBindData>();
	auto state = make_uniq_base<GlobalTableFunctionState, LanceSearchGlobalState>();
	...
	auto table_filters = BuildLanceTableFilterIRParts(bind_data.names, bind_data.types, input, true);
	if (bind_data.prefilter && !table_filters.all_prefilterable_filters_pushed) {
		auto function_name = bind_data.mode == LanceSearchMode::Fts ? "lance_fts" : "lance_hybrid_search";
		throw InvalidInputException(string(function_name) + " requires filter pushdown for prefilterable "
...
LanceSearchLocalInit(ExecutionContext &context, TableFunctionInitInput &input, GlobalTableFunctionState *global_state) {
	auto &global = global_state->Cast<LanceSearchGlobalState>();

	auto chunk = make_uniq<ArrowArrayWrapper>();
	auto result = make_uniq<LanceSearchLocalState>(std::move(chunk), context.client);
	result->column_ids = input.column_ids;
	result->filters = input.filters.get();
	result->global_state = &global;
	result->filter_pushed_down = global.filter_pushed_down;
...
static void LanceSearchFunc(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	if (!data.local_state) {
		return;
	}
	...
	while (true) {
		if (local_state.chunk_offset >= NumericCast<idx_t>(local_state.chunk->arrow_array.length)) {
			if (!LanceSearchLoadNextBatch(context, local_state, bind_data, global_state)) {
				return;
			}
		}
...
static InsertionOrderPreservingMap<string> LanceSearchBindToString(const LanceSearchBindData &bind_data) {
	InsertionOrderPreservingMap<string> result;
	result["Lance Path"] = bind_data.file_path;
	result["Lance Search Backend"] = bind_data.namespace_backed ? "namespace_query_table" : "dataset_scan";
	result["Lance Search Mode"] = bind_data.mode == LanceSearchMode::Fts ? "fts" : "hybrid";
	result["Lance K"] = to_string(bind_data.k);
	result["Lance Prefilter"] = bind_data.prefilter ? "true" : "false";
	result["Lance Dataset Cache Hit"] = bind_data.dataset_cache_hit ? "true" : "false";
	if (!bind_data.namespace_filter.empty()) {
...
static InsertionOrderPreservingMap<string> LanceSearchToString(TableFunctionToStringInput &input) {
	auto &bind_data = input.bind_data->Cast<LanceSearchBindData>();
	return LanceSearchBindToString(bind_data);
}

static InsertionOrderPreservingMap<string> LanceSearchDynamicToString(TableFunctionDynamicToStringInput &input) {
	auto &bind_data = input.bind_data->Cast<LanceSearchBindData>();
...
static void LanceSearchSerialize(Serializer &serializer, const optional_ptr<FunctionData> bind_data_p,
                                 const TableFunction &) {
	...
	serializer.WriteProperty(114, "oversample_factor", bind_data.oversample_factor);
	...
	if (bind_data.namespace_backed) {
		auto &cfg = bind_data.namespace_config;
		serializer.WriteProperty(119, "namespace_kind", static_cast<uint8_t>(cfg.kind));
		...
		serializer.WriteProperty(125, "namespace_replay_secret_name", cfg.replay_secret_name);
	}
}

static unique_ptr<FunctionData> LanceSearchDeserialize(Deserializer &deserializer, TableFunction &) {
	auto result = make_uniq<LanceSearchBindData>();
	auto &context = deserializer.Get<ClientContext &>();
	result->task_cpu_slots = MaxValue<idx_t>(1, context.db->NumberOfThreads());
	result->mode = static_cast<LanceSearchMode>(deserializer.ReadProperty<uint8_t>(100, "mode"));
	...
	result->namespace_backed = deserializer.ReadProperty<bool>(103, "namespace_backed");
	result->namespace_filter = deserializer.ReadProperty<string>(104, "namespace_filter");
	...
	result->vector_column = deserializer.ReadProperty<string>(107, "vector_column");
	result->vector_query = deserializer.ReadProperty<vector<float>>(108, "vector_query");
	...
	result->refine_factor = deserializer.ReadProperty<uint64_t>(111, "refine_factor");
	result->use_index = deserializer.ReadProperty<bool>(112, "use_index");
	result->alpha = deserializer.ReadProperty<float>(113, "alpha");
	result->oversample_factor = deserializer.ReadProperty<uint32_t>(114, "oversample_factor");
	result->k = deserializer.ReadProperty<uint64_t>(115, "k");
	result->dataset_version = deserializer.ReadProperty<uint64_t>(116, "dataset_version");
	...
	if (result->namespace_backed) {
		auto &cfg = result->namespace_config;
		cfg.kind = static_cast<LanceNamespaceKind>(deserializer.ReadProperty<uint8_t>(119, "namespace_kind"));
		...
		cfg.delimiter = deserializer.ReadProperty<string>(123, "namespace_delimiter");
		cfg.display_uri = deserializer.ReadProperty<string>(124, "namespace_display_uri");
		cfg.replay_secret_name = deserializer.ReadProperty<string>(125, "namespace_replay_secret_name");
		// Storage options, auth values and headers remain outside the plan.
	} else {
		if (result->dataset_version == 0) {
			throw SerializationException("Serialized Lance search is missing its fixed dataset version");
		}
		result->dataset_entry = LanceGetOrOpenDatasetEntryAtVersion(context, result->file_path, result->dataset_version,
		                                                            &result->dataset_cache_hit);
		result->dataset = result->dataset_entry ? result->dataset_entry->Handle() : nullptr;
		if (!result->dataset || lance_dataset_version(result->dataset) != result->dataset_version) {
			throw IOException("Failed to reopen fixed Lance search snapshot: " + result->file_path +
			                  LanceFormatErrorSuffix());
		}
	}
	PopulateSearchSchemaFromTypes(context, result->schema_root, result->arrow_table, result->names, result->types);
...
static void RegisterLanceFtsSearch(ExtensionLoader &loader) {
	TableFunction fts("lance_fts", {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR}, LanceSearchFunc,
	                  LanceFtsBind, LanceSearchInitGlobal, LanceSearchLocalInit);
...
static void RegisterLanceHybridSearch(ExtensionLoader &loader) {
	...
	TableFunction hybrid_f32("lance_hybrid_search",
	                         ...
	                         LanceSearchFunc, LanceHybridBind, LanceSearchInitGlobal, LanceSearchLocalInit);
