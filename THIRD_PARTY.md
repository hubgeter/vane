...
| lance-duckdb | Lance dataset SQL, namespace, search, mutation, index, and maintenance integration, fetched at a pinned revision and statically linked into `vane._native` | Apache-2.0 | `LICENSE` contains the applicable Apache-2.0 terms |
...
The Lance extension also statically links Rust crates resolved by the pinned
lance-duckdb `Cargo.lock`. Their license expressions and preserved license and
notice texts are reproduced in `LICENSES/lance-rust-dependencies.txt`,
generated by `scripts/sync_lance_cargo_licenses.py`. Pass `--manifest` or set
`DUCKDB_LANCE_DIRECTORY` to the exact lance-duckdb checkout when regenerating
the bundle. Regenerate and review it for every intentional Cargo manifest or
lock-file change. Vane does not carry local source patches for those crates.
