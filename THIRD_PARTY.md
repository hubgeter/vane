...
| lance-duckdb | Lance dataset SQL, namespace, search, mutation, index, and maintenance integration, statically linked into `vane._native` | Apache-2.0 | `external/lance-duckdb/LICENSE` |
...
The Lance extension also statically links Rust crates resolved by
`external/lance-duckdb/Cargo.lock`. Their license expressions and preserved
license/notice texts are reproduced in
`LICENSES/lance-rust-dependencies.txt`, generated by
`scripts/sync_lance_cargo_licenses.py`. Regenerate and review that bundle for
every intentional Cargo manifest or lock-file change. Three transitive storage
crates are retained as auditable source patches under
`external/lance-duckdb/vendor-patches`; their upstream archive hashes and
security-only dependency changes are recorded in that directory's README.
